...
def transmisi():
    while True:
        ...
        print('Masukkan data yang valid! (AT atau MT)') 
...
def update_stok():
    update_tabel = input('Masukan ID. Mobil yang ingin diupdate (Ketik 0 Jika Batal): ').capitalize()
    if update_tabel == 0:
        return 
...
def hapus_data():
    hapus_tabel = input('Masukan ID. Mobil yang ingin dihapus (Ketik 0 Jika Batal): ').capitalize()
    if hapus_tabel == 0:
        return 
    ...
    for item in mobil_list:
        ...
        return print("Mobil tidak ditemukan! Kembali ke menu.")
print(' ')


# 7. Menambah data Transaksi (ADDED FEATURE)
# Fungsi untuk melakukan filter dan sortir mobil
def filter_mobil():
    while True:
        print("\n=== Filter Mobil ===")
        ...
        if pilihan == 1:
            sort_order = validasi_alfabet("Urutkan harga dari rendah ke tinggi atau tinggi ke rendah? (rendah/tinggi): ").lower()
        ...
        # Filter berdasarkan transmisi
            ...
            transmisi_filter = validasi_alfabet("Masukkan jenis transmisi yang dicari (AT/MT): ").upper().strip()
            if transmisi_filter not in ["AT", "MT"]:
                print("Input tidak valid! Pilih AT atau MT.")
                continue
        ...
        # Urutkan berdasarkan kapasitas
            ...
            sort_order = validasi_alfabet("Urutkan kapasitas dari kecil ke besar atau besar ke kecil? (kecil/besar): ").lower()
            if sort_order == "kecil":
...
def tambah_transaksi():
    read_mobil()
    pesanan = input('Masukan ID. Mobil yang ingin disewa (Ketik 0 untuk batal): ').capitalize()
    if pesanan == "0":
        return
